...
import { readableStreamToString, ApiVideoError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ApiVideoError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ApiVideoError(e, e.response?.data)
                }
                if (innerError instanceof ApiVideoError) {
                    // Got "ApiVideoError" from the above try block
                    throw innerError;
                }
